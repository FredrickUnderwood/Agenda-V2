...
// fetchLocalMetrics GETs http://<backendHost>:<port><path> — the app
...
// to 127.0.0.1 but is configurable (host.docker.internal under DooD).
func fetchLocalMetrics(ctx context.Context, backendHost string, port int, path string) (body []byte, contentType string, err error) {
	...
	url := fmt.Sprintf("http://%s:%d%s", backendHost, port, path)
